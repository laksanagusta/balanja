...
test("cart is an accessible full-screen bottom drawer at every width", async () => {
  ...
  assert.match(css, /\.retail-pos-cart-open\s*\{[\s\S]*display:\s*flex[\s\S]*inset-block-end:\s*calc\(env\(safe-area-inset-bottom, 0px\) \+ 1rem\)[\s\S]*translate:[\s\S]*--app-bottom-navigation-progress/);
...
test("POS filters stay visible above the catalog while mobile cart floats above navigation", async () => {
  ...
  assert.match(css, /\.retail-pos-cart-open\s*\{[\s\S]*inset-block-end:\s*calc\(env\(safe-area-inset-bottom, 0px\) \+ 1rem\)[\s\S]*inset-inline-start:\s*50%[\s\S]*translate:[\s\S]*--app-bottom-navigation-progress/);
