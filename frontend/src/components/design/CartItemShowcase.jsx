...
function formatCartRowPrice(value) {
  return formatIDR(value).replace(/^Rp/, "");
}
        ...
        <div className="cart-item-list">
          {items.map((item) => (
            <CartRow
              key={item.name}
              item={item}
              subtotal={formatCartRowPrice(item.qty * item.price)}
              unitPrice={`${formatCartRowPrice(item.price)} / pcs`}
