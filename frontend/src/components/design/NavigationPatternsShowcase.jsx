    ...
    <Panel className="grid gap-5 p-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-text-muted">
          The authenticated shell uses a quiet white top bar with brand and account access, plus four primary workspaces and Lainnya in a floating five-item bottom navigation. The translucent pill sits a full 16px above the safe-area boundary and uses a soft downward shadow for separation. The active item receives a pill-shaped fill that slides between cells on route change. Once content scrolls beneath the bar, a masked frosted halo blends the blurred and clear regions without a hard edge; the pill progressively collapses while scrolling down and returns with sustained upward movement. Page floating actions ride the same movement, descending toward the bottom edge as the pill collapses and rising back above it when the pill returns, so the hidden state leaves no dead gap at the bottom. That response continues across route transitions, including the first gesture on each newly mounted page. Pointer route selection releases focus before the page changes so scrolling resumes immediately, while keyboard activation retains focus. The account avatar sits directly in a transparent 44px target without a visible wrapper surface. Transaksi and Laporan Penjualan remain available from the compact Lainnya sheet. The app canvas is centered and capped at 1200px on larger screens.
        </p>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-text-muted">
