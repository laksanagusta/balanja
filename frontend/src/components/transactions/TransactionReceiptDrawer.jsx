...
function paymentLabel(method = "") {
  ...
  return method ? method.toUpperCase() : "Tidak diketahui";
}
    ...
    >
      <Drawer.Portal>
        ...
        <Drawer.Content
          ...
          className="transaction-detail-drawer fixed inset-0 z-[80] m-auto flex h-[min(90svh,48rem)] max-h-[calc(100svh-1rem)] w-[min(28rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-[24px] border border-border bg-surface-muted px-2 py-2 outline-none shadow-panel sm:rounded-[28px] sm:px-3 sm:py-3"
        >
          ...
          <div className="transaction-receipt-paper-frame relative min-h-0 flex-1">
            <div className="transaction-receipt-paper relative h-full min-h-0 overflow-hidden rounded-card bg-surface">
              <div className="transaction-receipt-scroll relative min-h-0 h-full overflow-y-auto px-5 py-8 sm:px-8 sm:py-10">
                ...
                <dl className="mt-5 grid gap-2 rounded-card border border-border bg-surface-muted px-4 py-3 font-mono text-xs">
                  <div className="flex justify-between gap-4 text-text-muted">
                    <dt>Metode pembayaran</dt>
                    <dd className="font-semibold text-text">{paymentLabel(transaction.paymentMethod)}</dd>
                  </div>
                  <div className="flex justify-between gap-4 text-text-muted">
                    <dt>Kasir</dt>
                    <dd className="break-words text-right font-semibold text-text">{transaction.cashierName || "Tidak diketahui"}</dd>
                  </div>
                </dl>
                ...
                <div className="grid gap-3 font-mono text-sm">
                  {(transaction.items || []).map((item, index) => (
                    <div key={`${item.productId || "item"}-${item.variantId || ""}-${index}`} className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <p className="break-words font-medium text-text">{item.name}</p>
                        <p className="font-mono text-xs text-text-subtle">×{item.qty}</p>
                      </div>
          ...
          <footer className="transaction-receipt-controls relative z-10 grid shrink-0 px-5 pb-[max(0.75rem,env(safe-area-inset-bottom,0px))] pt-3 sm:px-8">
            <Drawer.Close asChild>
              <button
                type="button"
                className="transaction-receipt-close min-h-11 justify-self-center rounded-control px-4 text-sm font-semibold text-text-muted underline-offset-4 transition-[color,text-decoration-color,background-color] duration-fast ease-standard hover:bg-surface hover:text-text hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus"
              >
                Tutup
              </button>
            </Drawer.Close>
