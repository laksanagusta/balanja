...
Toast feedback uses `sonner` with a single root `Toaster`. Use success toasts for completed cashier actions such as barcode scans, product saves, cart clearing, settings saves, and completed sales. Use error toasts for validation, unknown barcodes, stock, and payment failures.

The global barcode scanner stays open after both successful and unsuccessful scans so cashiers can continue scanning without reopening it. Report each result through a success or error toast, keep the camera active, suppress duplicate reads from adjacent frames, and clear a submitted manual barcode for the next entry. The scanner closes only through its explicit close control. It is a dedicated full-screen scan workspace, not a modal card: the live camera feed should own the entire viewport edge to edge, keep the scan frame centered, and anchor title, close action, errors, and manual-entry controls in top and bottom gradients.
...
On compact screens, navigation opens as an anchored overlay with a dismissing scrim rather than pushing the current workspace downward. The trigger exposes its accessible name, expanded state, and controlled sheet; route selection, Escape, or the scrim closes the overlay. Reduced-motion contexts retain a short opacity change without spatial movement.
...
The Stock page follows the same operational table pattern and sits between Products and Transactions in the sidebar. It loads shared products for movement actions while movement history stays in a page-local server query. Keep stock search, movement type, and `Pergerakan baru` in the compact page header; do not add a separate filter modal unless there are more high-value filters. Use a full skeleton only before initial stock movement data exists; refetches should preserve the table and show a compact updating indicator. Manual movement dialogs require product, movement type, quantity, and reason, then preview current stock, signed delta, and stock after before submit. Product selection uses a debounced searchable picker backed by the products API with a six-item result limit; product rows expose name, barcode, category, and unit, while current stock appears only in the preview metrics to avoid duplicate stock reads. Stock history should prefer human-readable actor labels: the audit column uses `Nama user` and falls back to an internal user id only when no display name is available. Positive stock deltas use the success tone, sale/reduce deltas use danger text, and set-exact movements use a neutral type badge with signed delta text. Quantity inputs show thousand separators in the UI and submit plain integers to the API.
...
The operational dashboard lives at `/dashboard` inside the shared app shell. It uses four compact KPI cards, a wide-primary/narrow-secondary chart row, then a top-products and low-stock row. Use rolling 7-day and 30-day periods; calculate metrics only from completed local POS transactions and show the established empty-state pattern when a period has no sales. Low stock means an active product with 10 units or fewer. Operational dashboard labels, KPI titles, chart panel titles, and empty states should use Indonesian cashier-facing copy such as `Pendapatan`, `Transaksi selesai`, `Rata-rata transaksi`, `Stok menipis`, `Metode pembayaran`, and `Kelola stok`.

Products follows the same Indonesian operational-copy rule: page title, search placeholder, filters, table columns, empty states, and product-form dialogs should use direct labels such as `Produk`, `Kategori`, `Harga`, `Stok`, `Aktif`, `Tambah produk`, `Nonaktifkan`, and `Atur ulang filter`.
...
Retail product imagery inside public mockups should represent everyday UMKM shelf goods such as shampoo, snack, susu, air minum, tissue, and detergent rather than cafe-style menu photography, and may reuse the existing Unsplash-backed retail asset set already used by the POS demo data.
